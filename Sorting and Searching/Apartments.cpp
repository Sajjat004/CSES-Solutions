...
int main() {
    ...
    while (test--) {
        ...
        vi desiredSize(n, 0), appartment(m, 0);
        for (auto& x : desiredSize) x = Int();
        for (auto& x : appartment) x = Int();
        sort(all(desiredSize));
        sort(all(appartment));
        int res = 0;
        int i = 0, j = 0;
        while (i < n and j < m) {
            if (appartment[j] < desiredSize[i] - k) {
                j++;
            } else if (appartment[j] > desiredSize[i] + k) {
                i++;
            } else {
                res++;
                i++;
