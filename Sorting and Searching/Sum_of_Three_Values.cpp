...
bool get(int pos1, int pos2, int sum, int n) {
    int c = sum - v[pos1].ff - v[pos2].ff;
    int l = 0, r = n - 1;
    int pos3 = -1;
    while (l <= r) {
        int mid = (l + r) / 2;
        if (v[mid].ff < c) l = mid + 1;
        else if (v[mid].ff > c) r = mid - 1;
        else {
            pos3 = mid;
            l = mid + 1;
        }
    ...
    if (pos3 == -1) return 0;
    if (v[pos1].ss == v[pos3].ss or v[pos2].ss == v[pos3].ss) return 0;
    printf("%d %d %d\n", v[pos1].ss, v[pos2].ss, v[pos3].ss);
    return 1;
}
